globals [
  ...
  o2-level        ;; concentração de Oxigênio (O2) em porcentagem
]
...
to spread-fire
  ...
  ;; Propaga o fogo com maior probabilidade para vizinhos favorecidos
  if any? favored-neighbors [
    ask one-of favored-neighbors [ ignite ]
  ]
  ;; Propaga o fogo para outros vizinhos com menor probabilidade
  if any? other-neighbors [
    ask one-of other-neighbors [
      if random-float 1 < 0.5 [ ignite ]  ;; 50% de chance para outros vizinhos
    ]
  ]
...
;; Verifica se o patch está favorecido pela direção do vento
to-report wind-favor?
  ...
  report angle-diff <= 45 or angle-diff >= 315  ;; dentro de um cone de 90 graus
end

to fade-embers
  ask embers
    [ set color color - 0.3  ;; escurece o vermelho
      if color < red - 3.5  ;; verifica se está quase preto
        [ set pcolor color
          die ] ]
end
...
to update-air-composition
  ...
  ;; Ajuste dos cálculos:
  ;; CO: cada árvore queima 0.1 ppm de monóxido (ajuste mais conservador)
  set co-level co-level + (new-burned * 0.1)

  ;; CO2: cada árvore emite 10 ppm de dióxido de carbono (ajuste mais moderado)
  set co2-level co2-level + (new-burned * 10)

  ;; PM2.5: cada árvore gera 5 µg/m³ de partículas finas
  set pm2_5-level pm2_5-level + (new-burned * 5)

  ;; PM10: cada árvore gera 5 µg/m³ de partículas maiores
  set pm10-level pm10-level + (new-burned * 5)

  ;; O2: cada árvore consome 0.2 ppm de oxigênio (ajuste para ppm)
  set o2-level max (list (o2-level - (new-burned * 0.2)) 15000)  ;; O2 não pode cair abaixo de 15.000 ppm (aproximadamente 15%)
end
@#$#@#$#@
GRAPHICS-WINDOW
200
